...
export default function AdminFilterSelect({
  ...
  const isDefaultCategory = (cat: string) => {
    return DEFAULT_CATEGORIES.includes(cat.toLowerCase()) || cat === "all";
  };
  ...
  const handleDelete = async (e: React.MouseEvent, category: string) => {
    ...
    if (isDefaultCategory(category)) {
      alert("Cannot delete default categories (Business, Design, Manufacturing) or 'All'");
      return;
    }
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      <button
        ...
        className="w-full px-4 py-2 border-2 border-[#CBB49A] rounded-full text-sm focus:ring-2 focus:ring-[#CBB49A]/20 focus:border-[#CBB49A] outline-none transition-all bg-white text-[#2D2A2E] min-w-[180px] cursor-pointer hover:bg-[#F5EFE6] flex items-center justify-between"
      >
