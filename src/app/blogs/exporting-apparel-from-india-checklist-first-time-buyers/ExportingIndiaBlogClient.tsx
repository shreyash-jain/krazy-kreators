...
const MOCK_COMMENTS = [
  {
    ...
    comment:
      "Extremely helpful checklist! The documentation pointers and compliance notes are gold for first-time exporters.",
    date: "2 days ago",
    avatar: "S",
    likes: 34,
  },
  {
    ...
    comment:
      "The section on pre-shipment checks really saved me a lot of back-and-forth. Thanks for laying it out so clearly!",
    date: "1 week ago",
    avatar: "M",
    likes: 27,
  },
  {
    ...
    comment:
      "As someone new to exporting, this article made the process feel manageable. Bookmarked.",
    date: "2 weeks ago",
    avatar: "A",
    likes: 28,
  },
...
export default function ExportingIndiaBlogClient() {
  const [contactOpen, setContactOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [isLiked, setIsLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(() =>
    MOCK_COMMENTS.reduce((acc, c) => acc + (c.likes ?? 0), 0) || 67
  );
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const el = document.getElementById(`comment-${newData.id}`);
            ...
            <div className="prose prose-lg max-w-none">
              <div className="mb-12">
                <p className="text-lg text-[#666666] leading-relaxed mb-6">
                  Exporting apparel from India can be a complex process, especially for first-time buyers. This checklist covers essentials from preparation to post-shipment support.
                </p>
              </div>
              ...
              <div className="mb-12">
                ...
                <div className="mb-8">
                  ...
                  <ul className="list-disc list-inside text-lg text-[#666666] leading-relaxed space-y-2">
                    <li>Identify target-country requirements and seasonality.</li>
                    <li>Study duty rates, trade restrictions, and competitor sourcing regions.</li>
                  </ul>
                </div>
                <div className="mb-8 rounded-xl overflow-hidden shadow-lg">
                  <Image src="/blog/blog 5_1.png" alt="Market research" width={800} height={600} className="w-full h-auto object-contain" />
                </div>
              </div>

              <div className="mb-12">
                <h2 className="text-2xl font-bold text-[#2D2A2E] mb-6">Export Documentation</h2>
                <div className="mb-8 rounded-xl overflow-hidden shadow-lg">
                  <Image src="/blog/blog 5_2.png" alt="Export documentation" width={800} height={600} className="w-full h-auto object-contain" />
                </div>
                <ul className="list-disc list-inside text-lg text-[#666666] leading-relaxed space-y-2">
                  <li>Commercial Invoice, Packing List, and Certificate of Origin.</li>
                  <li>Export License (if applicable), Insurance, and LC/Payment docs.</li>
                </ul>
              </div>

              <div className="mb-12">
                <h2 className="text-2xl font-bold text-[#2D2A2E] mb-6">Quality & Pre-Shipment</h2>
                <div className="mb-8 rounded-xl overflow-hidden shadow-lg">
                  <Image src="/blog/blog 5_3.png" alt="Pre-shipment checks" width={800} height={600} className="w-full h-auto object-contain" />
                </div>
                <p className="text-lg text-[#666666] leading-relaxed">
                  Align AQL levels, run inline and final inspections, and verify packaging and labeling compliance.
                </p>
              </div>
                      ...
                      <>
                        ...
                        {comments.length > 3 && !showAllComments && (
                          <div className="text-center py-4">
                            <button onClick={() => setShowAllComments(true)} className="text-[#CBB49A] hover:text-[#b7a078] font-medium transition-colors duration-300">
                        ...
                        {comments.length > 3 && showAllComments && (
                          <div className="text-center py-4">
                            <button onClick={() => setShowAllComments(false)} className="text-[#CBB49A] hover:text-[#b7a078] font-medium transition-colors duration-300">
                              See Less
                            </button>
                          </div>
                        )}
                    ...
                    <form onSubmit={handleSubmitComment} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <input type="text" id="name" name="name" value={newComment.name} onChange={handleInputChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CBB49A] focus:border-transparent transition-all duration-300" placeholder="Enter your full name" required />
                        </div>
                        <div>
                          <label htmlFor="email" className="block text-sm font-medium text-[#2D2A2E] mb-3">Email Address *</label>
                          <input type="email" id="email" name="email" value={newComment.email} onChange={handleInputChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CBB49A] focus:border-transparent transition-all duration-300" placeholder="your.email@example.com" required />
                        </div>
                      </div>
                      <div>
                        <label htmlFor="comment" className="block text-sm font-medium text-[#2D2A2E] mb-3">Your Comment *</label>
                        <textarea id="comment" name="comment" value={newComment.comment} onChange={handleInputChange} rows={5} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#CBB49A] focus:border-transparent transition-all duration-300 resize-none" placeholder="Share your thoughts, questions, or feedback about this article..." required />
                        <p className="text-xs text-[#666666] mt-2">Your email will be visible to other readers. Please be respectful in your comments.</p>
                      </div>
                      <div className="hidden sm:flex items-center justify-between pt-4">
                        <div className="text-sm text-[#666666]"><span className="font-medium">{commentCount}</span> comment{commentCount !== 1 ? "s" : ""} so far</div>
                        <Button type="submit" disabled={isSubmitting || !newComment.name.trim() || !newComment.email.trim() || !newComment.comment.trim()} className="bg-[#CBB49A] text-white hover:bg-[#b7a078] px-8 py-3 rounded-lg font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                          {isSubmitting ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Posting...</>) : (<><MessageCircle className="w-4 h-4" /> Post Comment</>)}
                        </Button>
                      </div>
                      <div className="flex flex-col gap-4 sm:hidden pt-4">
                        <div className="text-sm text-[#666666] text-center"><span className="font-medium">{commentCount}</span> comment{commentCount !== 1 ? "s" : ""} so far</div>
                        <Button type="submit" disabled={isSubmitting || !newComment.name.trim() || !newComment.email.trim() || !newComment.comment.trim()} className="bg-[#CBB49A] text-white hover:bg-[#b7a078] px-8 py-3 rounded-full font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 w-full">
                          {isSubmitting ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Posting...</>) : (<><MessageCircle className="w-4 h-4" /> Post Comment</>)}
                        </Button>
                      </div>
                    </form>
