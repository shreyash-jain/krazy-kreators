...
const MOCK_COMMENTS = [
  { id: 1, name: "Sarah Chen", email: "sarah@example.com", comment: "Loved the flexibility tips for small runs — super relevant!", date: "2 days ago", avatar: "S", likes: 34 },
  { id: 2, name: "Marcus Rodriguez", email: "marcus@example.com", comment: "The batching idea + mixed SKUs is a game-changer.", date: "1 week ago", avatar: "M", likes: 27 },
  { id: 3, name: "Alex Thompson", email: "alex@example.com", comment: "Clear and practical — thanks for demystifying MOQs.", date: "2 weeks ago", avatar: "A", likes: 28 },
];

export default function MOQBlogClient() {
  const [contactOpen, setContactOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [isLiked, setIsLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(() => MOCK_COMMENTS.reduce((a, c) => a + (c.likes ?? 0), 0) || 83);
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const data = { id: comments.length + 1, name: newComment.name.trim(), email: newComment.email.trim(), comment: newComment.comment.trim(), date: "Just now", avatar: newComment.name.charAt(0).toUpperCase(), likes: 0 };
      setComments((p) => [data, ...p]); setCommentCount((p) => p + 1); setNewComment({ name: "", email: "", comment: "" }); setIsSubmitting(false); setShowSuccessMessage(true);
        ...
        <div className="min-w-[80%] lg:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="prose prose-lg max-w-none">
              <div className="mb-12">
                <p className="text-lg text-[#666666] leading-relaxed mb-6">MOQ (Minimum Order Quantity) often blocks new brands. Here’s how we make low-volume production viable without compromising quality.</p>
              </div>

              <div className="mb-12">
                <h2 className="text-2xl font-bold text-[#2D2A2E] mb-6">Flexible Order Quantities</h2>
                <p className="text-lg text-[#666666] leading-relaxed mb-6">Start with 20–50 pieces to test demand, scale when you’re ready.</p>
                <div className="mb-8 rounded-xl overflow-hidden shadow-lg">
                  <Image src="/blog/blog 6_1.png" alt="Flexible order quantities" width={800} height={600} className="w-full h-auto object-contain" />
                </div>
              </div>

              <div className="mb-12">
                <h2 className="text-2xl font-bold text-[#2D2A2E] mb-6">Mix-and-Match Production</h2>
                <p className="text-lg text-[#666666] leading-relaxed mb-6">Batch across styles and sizes to hit MOQs efficiently.</p>
                <div className="mb-8 rounded-xl overflow-hidden shadow-lg">
                  <Image src="/blog/blog 6_2.png" alt="Mix and match" width={800} height={600} className="w-full h-auto object-contain" />
                </div>
              </div>
